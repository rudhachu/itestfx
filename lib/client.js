          ...
          if (commit_key != Commit_key && Commit_key != "rudhra") {
            await personalDB(
              ...
              "set",
				...
				let description = commits['all']
 					   .map(commit => `_*Date:* ${commit.date.substring(0, 10)}_\n_*Message:* ${commit.message}_\n_*Committed by:* ${commit.author_name}_\n`)
 					   .join("\n");
					...
					if (description) {
   					 const updateMsg = {
    					    ...
    					    contextInfo: {
     					       externalAdReply: {
       					         title: "𝗨𝗣𝗗𝗔𝗧𝗘 𝗔𝗩𝗔𝗜𝗟𝗔𝗕𝗟𝗘",
      					          body: "ʀᴜᴅʜʀᴀ ᴜᴘᴅᴀᴛᴇ ᴀᴠᴀɪʟᴀʙʟᴇ",
      					          sourceUrl: "https://github.com/princerudh/rudhra-bot",
       					         mediaUrl: "https://github.com/princerudh/rudhra-bot",
         					       mediaType: 1,
 					   ...
 					   await conn.sendMessage(sudo, {
       					 text: GenListMessage(updateMsg, ['update now'], '_Reply to this message and send one(1) if you want update_')
   					 });
					}
				}
			} catch (e) {}
