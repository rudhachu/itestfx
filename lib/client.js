          ...
          if (commit_key != Commit_key && Commit_key != "rudhra") {
            await personalDB(
              ...
              "set",
					...
					const update_msg = "𝗥𝗨𝗗𝗛𝗥𝗔 𝗨𝗣𝗗𝗔𝗧𝗘 𝗔𝗩𝗔𝗜𝗟𝗔𝗕𝗟𝗘";
					let description = "";
					commits['all'].map(commit => {
						description += `*Date:* ${commit.date.substring(0, 10)}\n*Message* ${commit.message}\n*Commited by:* ${commit.author_name}\n\n`;
					});
					if (description) {
						const sudo = config.SUDO ? (typeof config.SUDO === 'string' ? numToJid(config.SUDO.split(",")[0]) : numToJid(config.SUDO.toString())) : conn.user.id;
						await conn.sendMessage(sudo, {
							text: GenListMessage(update_msg, ['update now'], description, 'ʀᴇᴘʟʏ ᴛᴏ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴀɴᴅ ꜱᴇɴᴅ ᴏɴᴇ 𝟭 ɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴜᴘᴅᴀᴛᴇ')
						});
					}
				}
			} catch (e) {}
